...
The 2026-09-10 art-direction brief supersedes the former light-motion-only, no-new-images and fixed-desktop-layout limits. Page surfaces remain flat, separated by neutral fills and fine rules. The hero photographs have restrained offset shadows (`8px 18px 34px #20262214`); CSS perspective and transform depth separate photographs without adding card UI. The booking dialog retains its existing shadow and backdrop. Service rows stay stationary.

GSAP 3.15 and ScrollTrigger load dynamically only at width ≥1100px, height ≥650px, with hover/fine pointer and no reduced-motion preference. The page remains a Server Component. CSS/native Web Animations continue to own existing light mobile/booking feedback; they never animate the same desktop photo element as GSAP. A single gallery pin consumes 1250px of native vertical scrolling, without wheel interception, snap or nested pins. Header and booking ancestors are not transformed.
...
- **Desktop photography:** three hero planes arrive from separate positions/depths over 900ms, retaining visible main photography and text, then react to pointer movement and scroll at different speeds. Nested transforms separate entrance, scroll and pointer ownership. Team portraits enter at different offsets and respond gently; text/buttons stay still. Five gallery photographs travel through a horizontal curve with rotation and depth. The active title/detail sit below the central image; Previous/Next, a count, progress and Skip remain stable. Offstage frames are inert and hidden from assistive technology. Visible planes stay opaque to avoid ghosting. The default static gallery shows all five photographs. Space ribbons have opposite 34s/41s loops, offset depth and white-fading top/bottom masks. The space copy never pins.
- **Motion control and cleanup:** the header's “Dừng tự chạy” pauses autonomous motion and pointer tweens; scrolling remains user-controlled. Loops pause outside the viewport, when booking is open, or on document visibility loss. Media changes/unmount revert GSAP before restoring ARIA/inert, removing transforms, visibility and the pin spacer. Desktop extras use media-qualified picture sources, so mobile/touch does not request them. Two hero extras load eagerly only on eligible desktop; other extras load lazily. Seven new WebP files total 311,566 bytes, with sources, license links and reproducible crops in `public/images/motion/sources.json` and `scripts/prepare-motion-assets.mjs`.
